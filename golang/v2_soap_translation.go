...
func main() {
	http.HandleFunc("/v2_soap_translation", func(w http.ResponseWriter, r *http.Request) {
		n := r.URL.Query().Get("n")
		if n == "" {
  ...
  <soap:Body>
    <NumberToWords xmlns="http://www.dataaccess.com/webservicesserver/">
      <ubiNum>%s</ubiNum>
		...
		if err != nil {
			w.WriteHeader(http.StatusInternalServerError)
			fmt.Fprint(w, "Error al leer la respuesta SOAP")
			return
		}
		...
		if len(match) > 1 {
			englishWord := strings.TrimSpace(match[1])
			...
			if err != nil {
				w.WriteHeader(http.StatusInternalServerError)
				fmt.Fprint(w, "Error al traducir")
				return
			}

			w.Header().Set("Content-Type", "text/plain; charset=utf-8")
			fmt.Fprint(w, strings.ToLower(translated))
		} else {
			w.WriteHeader(http.StatusInternalServerError)
